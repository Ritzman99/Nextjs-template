...
export async function POST(
  ...
  try {
    ...
    const requesterId = msg.fromRef;
    const isRecipient = msg.toRefs.some(
      (t) => t.type === 'user' && t.ref.equals(userObjectId)
    );
    if (!isRecipient) {
      return NextResponse.json(
        { error: 'Only the recipient can accept this friend request' },
        { status: 403 }
      );
    }

    await UserContactModel.updateOne(
      { ownerId: requesterId, contactUserId: userObjectId },
      { $set: { state: 'friend' } },
      { upsert: true }
    );
    await UserContactModel.updateOne(
      { ownerId: userObjectId, contactUserId: requesterId },
      { $set: { state: 'friend' } },
      { upsert: true }
    );
    ...
    await UserConversationStateModel.updateOne(
      ...
      { $set: { folder: 'inbox', readAt: new Date() } }
    );

    return NextResponse.json({ ok: true });
  } catch (e) {
    console.error('POST /api/inbox/conversations/[id]/accept-friend-request:', e);
